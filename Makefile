...
dev:
	...
	docker run -d --name gopizza_db --network host -e POSTGRES_DB=gopizza -e POSTGRES_USER=user -e POSTGRES_PASSWORD=pass -v gopizza_data:/var/lib/postgresql/data postgres:15
	@sqlc generate
